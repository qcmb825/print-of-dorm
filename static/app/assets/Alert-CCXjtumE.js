...
`,[w(`border`,`
 ...
 padding-right: 24px;
 `)])]),w(`icon`,{color:`var(--n-icon-color)`}),l(`alert-body`,{padding:`var(--n-padding)`},[w(`title`,{color:`var(--n-title-text-color)`}),w(`content`,{color:`var(--n-content-text-color)`})]),A({originalTransition:`transform .3s var(--n-bezier)`,enterToProps:{transform:`scale(1)`},leaveToProps:{transform:`scale(0.9)`}}),w(`icon`,`
 position: absolute;
 left: 0;
 top: 0;
 align-items: center;
