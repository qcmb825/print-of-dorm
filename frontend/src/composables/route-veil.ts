 ...
 *  同一时刻可能出现两套幕布、两条套准线。所以状态收在这里，覆盖层只在 App.vue 渲染一次。
 *
 *  状态机只有三态：idle → out（幕布咬合）→ in（停留、换手、开幕）→ idle。
 ...
 *  （幕布不位移、只留一次缓慢的明暗过渡）。少一处可能和 CSS 漂移的状态。
...
export interface VeilTarget {
  /** 等宽大写的代号：外壳档是 GUEST / STUDENT / STAFF，子页档兜底用目标路径 */
  code: string
  /** 目标页名，中文 */
  title: string
}
 ...
 *  重入时合闸不能再留那一段空拍（--veil-latch）：幕布多半已经在半开位置，
...
export function useRouteVeil() {
   ...
   *  写进了 props，换手时挂上的是**最新**那棵树。于是幕布上写着 A、幕布后面出现的是 B，
