...
**表格型的回复会渲染成一张卡**（订单 / 工单 / 预设 / 我的）：服务端用 Pillow 把数据画成
PNG（`botcard.py` + `GET /api/bot/card`），printbot 收到后当图片发出去，风格跟着网页端那套
深色工业配色走。一句话能说清的内容（取件码、下单成功、追问）仍然是文字 —— 手机里点开一张图
比读一行字慢。**服务器没装中文字体时自动退回纯文字**（接口回 501），不会发出一张全是方块的图；
要指定字体就设 `.env` 的 `CARD_FONT_PATH` / `CARD_MONO_FONT_PATH`。
