...
界面是响应式的（手机、平板、桌面各一套布局）。**界面锁定亮色**（2026-09-21 起：
不再跟随系统、也不提供切换入口）—— 令牌里那套深色仍在，只是没有入口。
视觉语言是克制的黄 + 青、细边框、磨砂面板，动效只用在真正需要提示变化的地方。

---

## 📌 版本

**v3**（2026-09-21）—— 这一版把「报价」这件事从一张纸搬进了系统，并让机器人成为正经的下单入口：

- **价目表**：一条 = 纸张 + 类型 + 单面价 + 双面价 + 备注，学生下单直接选一档。
  条目、单价、全局系数与说明**全部由管理员维护，没有任何写死的数值**；
  「双面价留空」表示这一档不支持双面（相纸那一档就是）。
- **自动估价**：下单时按**文件页数**算一个预估价格给学生看（PDF / Word / 图片都数得出来），
  最终金额仍然由接单人看过文件后自己确认 —— 预估价只是参考，两列数据互不干涉。
- **QQ 机器人引导**：右下角悬浮提示引导用机器人下单，点满三次自动收进设置页（二维码在那里，
  由超管上传）；机器人侧也有「价目表」卡片与「档位 → 单双面 → 份数」的下单流程。
- **订单来源**成了订单自己的字段（网页 / QQ 机器人），备注只放学生自己写的话。
- 界面：订单台不再横向滚动、开屏与换场动画按「加载快就播满、加载慢就跟着走」的节奏走、
  强制亮色、订单台跟随鼠标的抖动也去掉了。
...
| QQ 卡片渲染 | Pillow（`botcard.py`，只画固定版式的列表卡；不引无头浏览器） |
...
`config.ORDER_STATUSES_MANUAL`（待打印 / 打印中 / 可取件 / 已取件），不含待计费 ——
否则一笔已收钱的单会被改回「待计费」，对账就乱了。接口收到待计费会直接 400。
**「已取件」可以被管理员改回去**（例如改回「可取件」），这是有意的运维约定，不是疏漏：
...
**表格型的回复会渲染成一张卡**（订单 / 工单 / 打印服务 / 我的 / 使用说明）：服务端用 Pillow 把数据画成
PNG（`botcard.py` + `GET /api/bot/card`），printbot 收到后当图片发出去。风格是**深色工业 + 磨砂玻璃**：
深底 `#101110`、信号黄 `#FFFA00` 只做点缀、面板是一块浮在光斑背景上的玻璃（背景模糊 + 半透明 + 1px 描边 +
高光），母题取自《明日方舟：终末地》那套 UI 语言（细网格、刻度条、斜纹带、粗规线、等宽数字、
...
| `site_assets` | 全站一张的资产（v21 加，只有 `id = 1` 这一行）：目前就 QQ 机器人二维码一个文件名。存**文件名**、目录由 `ASSET_FOLDER` 决定，与收款码同一个道理 —— 换盘改目录时不会静默失联 |
...
│   ├── price_table.py     #   价目表（一条 = 纸张 + 类型 + 单面/双面单价；只影响预估价）
│   ├── bot_hint.py        #   QQ 机器人引导（悬浮提示的出现规则 + 全站那张二维码）
