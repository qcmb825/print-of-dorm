...
「需人工通知」这个标记**不落库**，而是订单台按「状态 = 可取了 且 学生联系方式
...
def _claim(conn, order_id):
    ...
    WHERE 里再判一次 status = 可取了，不是啰嗦：从上面那次 SELECT 到这里，
...
def _claimer_recipients(conn, claimer_id, count):
    ...
    if claimer_id:
        ...
        return []
    # 没有接单人：超管可以直接把没接单的订单改成「可取了」（改状态那道
